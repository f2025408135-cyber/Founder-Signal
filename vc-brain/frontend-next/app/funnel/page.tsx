...
function FunnelStage({ label, count, width, color, sublabel }: { label: string; count: number; width: string; color: string; sublabel: string }) { return <div className="flex flex-col items-center"><div className={cn("rounded-md p-4 text-center text-text-primary", color)} style={{ width }}><div className="mb-1 text-xs font-bold uppercase tracking-wider">{label}</div><div className="font-mono text-2xl font-bold">{count}</div><div className="mt-1 text-[10px] text-text-primary">{sublabel}</div></div></div>; }
