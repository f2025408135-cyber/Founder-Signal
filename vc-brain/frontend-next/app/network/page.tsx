...
export default function NetworkPage() {
  ...
  return <AppShell><div className="flex h-full flex-col"><header className="border-b border-border px-8 py-4"><p className="technical-label">Evidence provenance</p><h1 className="text-2xl font-bold tracking-tight text-text-primary">Sourcing Map</h1><p className="mt-1 text-sm text-text-muted">Founders connected to the channels that surfaced their evidence.</p></header>{isLoading ? <NetworkSkeleton /> : errors.length > 0 && inboundCards.length === 0 && outboundFounders.length === 0 ? <StateMessage tone="error" message="The sourcing map could not be loaded. Check that the backend is running, then retry." action={retry} /> : <>{errors.length > 0 && <StateMessage tone="warning" message="Some sourcing data is unavailable. The map below contains only the sources that loaded." action={retry} />}{inboundCards.length + outboundFounders.length === 0 ? <StateMessage tone="empty" message="No sourcing relationships yet. An isolated founder will appear here after the first application is received." /> : <NetworkCanvas inbound={inbound.data!} outbound={outboundFounders} />}</>}<div className="flex items-center gap-4 border-t border-border px-8 py-3 text-[10px] text-text-muted"><span className="flex items-center gap-1"><span className="h-3 w-3 rounded-full border border-border-strong" /> Founder</span><span className="flex items-center gap-1"><span className="h-3 w-3 rounded-full bg-success" /> Channel</span><span className="flex items-center gap-1"><span className="h-3 w-3 rounded-sm border border-warning-border" /> Cold-start</span></div></div></AppShell>;
}
